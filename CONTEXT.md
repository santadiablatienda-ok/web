...
## Qué se hizo (sesión 2026-07-30, continuación)

1. **Sección "Sale" con banner propio**, reemplazando el carrusel automático por descuento: `BANNERS/BANNER DE SALE.webp` (copiado a `public/banners/banner-sale.webp`) + `components/sale-carousel.tsx`. En desktop el carrusel de un producto por vez queda **integrado dentro del espacio en blanco del banner** (medido a pixel con un script: el blanco va del 28.65% al 66.98% del ancho de la imagen de 3840x608 — si se cambia el banner por otro con otras proporciones, hay que remedir y actualizar `BLANK_LEFT_PCT`/`BLANK_WIDTH_PCT` en ese archivo). En mobile el banner queda muy bajo de alto para superponer texto legible, así que el mismo carrusel se muestra debajo, a todo lo ancho, con fondo oscuro. Auto-avanza cada 3.8s, un producto a la vez, con puntitos de posición.
2. **Campo nuevo `on_sale` en `products`** (columna boolean, default false) — controla manualmente qué productos entran a esta sección, independiente del catálogo general. Requirió una migración SQL corrida a mano en el dashboard de Supabase (`alter table products add column if not exists on_sale boolean not null default false;`) — **Claude no tiene service-role key ni acceso a DDL**, así que cualquier cambio de estructura de tabla futuro va a necesitar este mismo paso manual.
3. **Pestaña "Sale" en el admin** (`SaleTab` en `app/admin/page.tsx`): tabla con buscador, toggle "En sale" y campo de % de descuento editable por fila, sin tener que entrar al form completo de cada producto.
4. **Fix de datos**: "Botas Sira" (bot-019, con precio/specs reales pero sin fotos) y "Sira" (brc-050, categorizado como "borcegos" pero con las fotos reales) eran el mismo producto duplicado — se unificaron en `brc-050` (se quedó con las fotos) tomando el resto de los datos de `bot-019`, que se borró. Incidente similar a tener en cuenta: **antes de cargar un producto nuevo desde catálogos/fotos externas, buscar por nombre si ya existe** para no duplicar.
5. **Corrección de precios/nombres a pedido del usuario**: "Milano" (brc-034, antes a $49.600) pasó a llamarse simplemente "Milano" con precio corregido a $60.000; el que se llamaba "Botas Milano" (bot-015, $77.500) pasó a llamarse "Milano Gamuza". "Botas Sira" (brc-050) se renombró a solo "Sira". Los 5 productos de esta primera tanda de Sale (Sira, Wanda, Angie, Arlet, Milano) quedaron con 10% de descuento y `on_sale: true`. **Arlet (prod-7m1epsa) no tiene foto cargada todavía** — se activó igual a pedido del usuario, pero muestra "Foto pronto" hasta que se le suba una imagen real.
6. Nota de proceso: para los cambios de datos en vivo (unificar productos, tildar `on_sale`, corregir precios) Claude ejecutó los writes vía `fetch` autenticado corrido en la consola del navegador del usuario, aprovechando una sesión de admin ya logueada (nunca tocó ni vio la contraseña) — no hay ningún atajo tipo service-role key guardado en el repo.
